...
function AppContent() {
  const {
    ...
    lastDeleted,
    clearLastDeleted,
    // ✅ pull raw tasks so we can read old values for detailed activity
    tasks,
  } = useTasksContext();
  ...
  // ✅ Detailed update log (with per-field diffs)
  const handleUpdate = useCallback(
    (id: string, patch: Partial<Task>) => {
      ...
      if (before) {
        ...
        if (
          patch.timeTaken !== undefined &&
          before.timeTaken !== after.timeTaken
        ) {
      ...
      // Use your requested snippet verbatim (includes details array)
      setActivity((prev) =>
        [
          {
            ...createActivity("update", `Edited: ${parts.join(", ")}`),
            details: parts,
          } as any,
          ...prev,
        ].slice(0, 50)
    ...
    [updateTask, createActivity, tasks]
  );
  ...
  const handleDelete = useCallback(
    (id: string) => {
      ...
      deleteTask(id);
      const name = target?.title ?? "Unknown";
      setActivity((prev) =>
        [
          createActivity("delete", `Deleted "${name}" having id ${id}`),
          ...prev,
        ].slice(0, 50)
      );
    },
    [deleteTask, createActivity, tasks]
  );
